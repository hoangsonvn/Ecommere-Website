...
ResourceBundle resourceBundle = ResourceBundle.getBundle("messages");
    ...
    public String login(HttpServletRequest request, @RequestParam(name = "err", required = false) String err) {
        ...
        if (roles.contains("ROLE_MANAGER") || roles.contains("ROLE_ADMIN") || roles.contains("ROLE_STAFF") || roles.contains("ROLE_EDITOR")) {
            return "redirect:/admin/home";
        }
        if (roles.contains("ROLE_USER")) {
            return "redirect:/client/home";
        }
        String verifySuccess = request.getParameter("verifySuccess");
       if(verifySuccess != null){
           request.setAttribute("verifySuccess",resourceBundle.getString(verifySuccess));
       }
        return "authen/login";
